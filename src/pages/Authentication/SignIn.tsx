...
const SignIn: React.FC = () => {
  ...
  const handleLogin = () => {
    ...
    // Nếu xác thực thành công, chuyển hướng đến "/admin"
    history('/admin/movies');
  };

  const handleLoginToastRegister = (strMessage: string) => {
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      const response = await axios.post('http://localhost:8080/user/login-admin', formData);
      ...
      if (response.status === 200) {
        ...
        const adminAcc = { username: response.data, isLogin: true };
        sessionStorage.setItem('admin', JSON.stringify(adminAcc));

        setTimeout(() => {
          handleLogin()
        }, 1500);
      } else if (response.status === 404) {
        handleLoginToastRegister("ERR_BAD_REQUEST")
      }
      ...
      if (error.code === "ERR_BAD_REQUEST") {
        ...
        handleLoginToastRegister("ERR_NETWORK")
        ...
        <div className="flex flex-wrap items-center">
          <div className="hidden w-full xl:block xl:w-1/2">
            <div className="py-17.5 px-26 text-center">
              <p className="mb-5.5 inline-block">
