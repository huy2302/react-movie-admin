...
import { getActors, getMovieDetails } from '../../function/ApiFunction';
import { Genre } from '../../Modals/genre';
import { Movie } from '../../Modals/movie';
import { Link } from 'react-router-dom';
import { Actor } from '../../Modals/actor';
import { Celebrity } from '../../Modals/celebrity';
...
interface DropdownProps {
    id: string;
    movieID: number;
    onSelectGenre: (options: Option[]) => void;
}

const MultiSelectActor: React.FC<DropdownProps> = ({ id, movieID, onSelectGenre }) => {
    ...
    const [genres, setActors] = useState<Actor[]>([])
    const [genresMovie, setActorsMovie] = useState([])
    ...
    // useEffect(() => {
    //     // Gọi hàm select khi genres được cập nhật
    //     if (genresMovie.length > 0) {
    //         genresMovie.map((item, index) => {
    //             console.log(index + " --- " + item)
    //             // select(index, {} as React.MouseEvent);
    //         })
    //     }
    // }, [genresMovie]);
    useEffect(() => {
    ...
    const select = (index: number, event: React.MouseEvent) => {
        ...
        onSelectGenre(selectedOptions);
        setOptions(newOptions);
    };
    // const setSelectM = (index: number) => {
    //     const newOptions = [...options];
    ...
    return (
        <div className="relative z-50">
            <label className="mb-3 block text-black dark:text-white">
                Diễn viên: {
                    genresMovie && genresMovie.length > 0 && genresMovie.map((item, index) => {
                        return (
                            <>
                                <p
                                    key={index*46}
                                    className="inline-flex items-center justify-center rounded-md border border-black mx-1 px-10 text-center font-medium text-black hover:bg-opacity-90 lg:px-8 xl:px-10"
                                >
                                    {item}
                                </p> 
                            </>
...
export default MultiSelectActor;
